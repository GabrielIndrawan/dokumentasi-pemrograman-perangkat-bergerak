...
{	//date('Y-m-d', strtotime($tanggal))
	...
	//echo "$tgl1";
	$query = "SELECT * FROM v_jual where date(tgl_jual)>=date('$tgl1') and date(tgl_jual)<=date('$tgl2') order by tgl_jual desc";
	if($tgl1=="1970-01-01")
	{
		$query = "SELECT * FROM v_jual order by tgl_jual desc";
	}
}
	else
	{
  ...
  //echo "<a href='tambahbarang.php'>Add</a>";
 	while ($hasil = mysqli_fetch_array ($sql)) {
		...
		$satuan = stripslashes ($hasil['kd_kons']);
		$harga = $hasil['nm_kons'];
